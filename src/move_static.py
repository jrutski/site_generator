    ...
    for dir_obj in os.listdir(dir_path_content):
        if os.path.isfile(os.path.join(dir_path_content, dir_obj)):
            if dir_obj.endswith('.md'):
                ...
                generate_page(os.path.join(dir_path_content, dir_obj), template_path, os.path.join(dest_dir_path, new_filename))
            else:
                print(f"{dir_obj} is not Markdown. Moving on.")
        else:
            generate_pages_recursive(os.path.join(dir_path_content, dir_obj), template_path, os.path.join(dest_dir_path, dir_obj))